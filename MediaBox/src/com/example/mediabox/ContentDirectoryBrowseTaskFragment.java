...
public class BrowseRegistryListener extends DefaultRegistryListener {
	...
	public void deviceAdded(Device device) {
		DeviceModel deviceModel = new DeviceModel(R.drawable.ic_device,
				device);
			...
			if (prefs.getBoolean("settings_validate_devices", false)) {
				if (device.isFullyHydrated())
					mService.getControlPoint().execute(
							new CustomContentBrowseTestCallback(device,
									conDir));
			} else {
				if (mCallbacks != null)
	...
	public void deviceRemoved(Device device) {
		if (mCallbacks != null)
			mCallbacks.onDeviceRemoved(new DeviceModel(
					R.drawable.ic_device, device));
	}
}
...
private class CustomContentBrowseActionCallback extends Browse {
	...
	private ItemModel createItemModel(DIDLObject item) {
		...
		URI usableIcon = item
				.getFirstPropertyValue(DIDLObject.Property.UPNP.ICON.class);
		if (usableIcon != null)
	...
	public void received(final ActionInvocation actionInvocation,
		...
		try {
			...
			actionInvocation.setFailure(new ActionException(
					ErrorCode.ACTION_FAILED, "Can't create list childs: "
							+ ex, ex));
	...
	public void failure(ActionInvocation invocation, UpnpResponse response,
			String s) {
		if (mCallbacks != null)
			mCallbacks.onDisplayItemsError(createDefaultFailureMessage(
					invocation, response));
	}
}
...
private class CustomContentBrowseTestCallback extends Browse {
	...
	public void received(final ActionInvocation actionInvocation,
			final DIDLContent didl) {
		if (mCallbacks != null)
			mCallbacks.onDeviceAdded(new DeviceModel(R.drawable.ic_device,
					device));
	}
	...
	public void navigateTo(Object model) {
		if (model instanceof DeviceModel) {
			...
			if (device.isFullyHydrated()) {
				...
				Toast.makeText(mActivity, R.string.info_still_loading,
		...
		if (model instanceof ItemModel) {
			...
			if (item.isContainer()) {
				if (mFolders.isEmpty())
					...
					mFolders.push(item);
				mService.getControlPoint().execute(
						new CustomContentBrowseActionCallback(
								item.getService(), item.getId()));
			} else {
				try {
					...
					Toast.makeText(mActivity,
							R.string.info_could_not_start_activity,
							Toast.LENGTH_SHORT).show();
				} catch (ActivityNotFoundException ex) {
					Toast.makeText(mActivity, R.string.info_no_handler,
							Toast.LENGTH_SHORT).show();
				}
			}
		}
		...
		if (mIsShowingDeviceList != null && mIsShowingDeviceList) {
			...
			if (!mFolders.empty()) {
				...
				if (mCurrentDevice != null) {
					...
					if (service != null)
						mService.getControlPoint().execute(
								new CustomContentBrowseActionCallback(service,
										"0"));
				}
			}
		}
